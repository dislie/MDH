...
class Aircraft(Dataset):
    ...
    @staticmethod
    def init(root):
        image_class_labels = pd.read_csv(os.path.join(root, 'data/variants.txt'), names=['target'])
        d = {}
        for i in range(len(image_class_labels)):
            d[image_class_labels['target'][i]] = i+1

        images_train = pd.read_csv(os.path.join(root,'data/images_variant_trainval.txt'), names=['filepath'])
        images_test = pd.read_csv(os.path.join(root,'data/images_variant_test.txt'), names=['filepath'])
        train_images = []
        test_images = []
        ...
        images_train = []
        for i in range(len(train_images)):
            images_train.append([img_id_train[i], 'data/images/'+train_images[i], label_list_train[i]])
        ...
        images_test = []
        for i in range(len(test_images)):
            images_test.append([img_id_test[i], 'data/images/'+test_images[i], label_list_test[i]])
